...
Open **http://127.0.0.1:8000**. Use **Load button + LED demo** immediately, without an API key. The venv and dependencies are already installed on the current laptop.
...
Scope: one 5 V source, one red LED, one 220 Ω resistor, jumpers, and optionally a momentary button. Free-text prompts are interpreted within this scope; blinking, latching, motors, multiple LEDs and other values are rejected. This is a limited electrical graph validator, not a general circuit simulator or physical inspection system. Prompt interpretation itself is model-based. The button-LED path has passed a live OpenRouter check; additional prompts still need evaluation.
...
- The raw board declares dimensions 165 × 10 × 55 mm, but its hole extents are X = −5.08…43.18 mm and Z = 0…157.48 mm. Coordinates are preserved exactly; dimensions are not used to place components or scale the board mesh.
